...
while ($i < $imaxinloop) {
	...
	if ($mode == 'kanban') {
		if ($i == 0) {
			print '<tr class="trkanban status'.$objectstatic->statut"><td colspan="'.$savnbfield.'">';
		...
		print '<tr class="oddeven '.((getDolGlobalInt('MAIN_FINISHED_LINES_OPACITY') == 1 && $obj->status > 1) ? 'opacitymedium' : '').' status'.$objectstatic->statut">';
